...
//					builder.WriteString(fmt.Sprintf("[%s] DNS (%s)\t%s\t%s", interaction.FullId, interaction.QType, interaction.RemoteAddress, interaction.Timestamp.Format("2006-01-02 15:04:05")))
					...
					println("************************************************")
					builder.WriteString(fmt.Sprintf(":globe_with_meridians: [%s] (*%s* :flag-%s:) %s (DNS %s)", interaction.Timestamp.Format("02/01/2006 15:04"), interaction.RemoteAddress, ip2country.GetCountry(interaction.RemoteAddress), interaction.FullId, interaction.QType))
					if cliOptions.Verbose {
...
//					builder.WriteString(fmt.Sprintf("[%s] Received HTTP interaction from %s at %s:\n%s", interaction.FullId, interaction.RemoteAddress, interaction.Timestamp.Format("2006-01-02 15:04:05"), interaction.RawRequest))
					builder.WriteString(fmt.Sprintf(":dart: [%s] (*%s* :flag-%s:) %s (HTTP) ```%s```", interaction.Timestamp.Format("02/01/2006 15:04"), interaction.RemoteAddress, ip2country.GetCountry(interaction.RemoteAddress), interaction.FullId, modifiedResponse))
